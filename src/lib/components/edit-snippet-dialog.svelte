<script lang="ts">
	...
	let {
		...
		form: SuperValidated<Infer<UpdateSnippetSchema>>;
	} = $props();
	...
	const superform = superForm(form, {
		validators: zod4Client(updateSnippetSchema),
	});
	const { form: formData, enhance } = superform;

	// Initialize form with snippet data when dialog opens
	$effect(() => {
		if (open && snippet) {
			formData.set({
...
<Dialog.Root bind:open>
	<Dialog.Content class="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
		<Dialog.Header>
			<Dialog.Title>Edit Snippet</Dialog.Title>
			<Dialog.Description>
				Update the details of your code snippet below.
			</Dialog.Description>
		</Dialog.Header>
		<form method="POST" action="?/update" use:enhance>
			<input type="hidden" name="snippetId" value={$formData.snippetId} />
